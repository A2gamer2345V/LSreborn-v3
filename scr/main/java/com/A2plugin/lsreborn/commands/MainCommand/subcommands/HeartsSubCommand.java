...
import com.A2plugin.lsreborn.util.MaxHeartsManager;
...
public final class HeartsSubCommand implements SubCommand {
    ...
    public HeartsSubCommand(LSReborn plugin) {
        this.plugin = plugin;
        ...
        for (OfflinePlayer targetPlayer : targetPlayers) {
            ...
            switch (optionTwo) {
                case "add": {
                    ...
                    if (targetPlayer instanceof Player) LSReborn.setMaxHealth((Player) targetPlayer, targetPlayerData.getMaxHealth());
                    break;
                }
                case "set": {
                    if (amount == 0) {
    ...
    public String getUsage() {
        return "/lsreborn hearts <add | set | remove> <player> [amount]";
    }

    @Override
    public boolean hasPermission(CommandSender sender) {
        return sender.hasPermission("lsreborn.admin.setlife");
    }
}
